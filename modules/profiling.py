...
class DataProfiler:
    ...
    def generate_profile_report(self, data: pd.DataFrame, 
        ...
        try:
            if not HAS_YDATA or ProfileReport is None:
                # Generate basic HTML summary report fallback
                if output_path is None:
                    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix='.html')
                    output_path = temp_file.name
                    temp_file.close()

                stats = self.get_basic_statistics(data)
                html_content = self._generate_fallback_html(data, stats, title or "Data Profile Report")
    ...
    def _generate_fallback_html(self, df: pd.DataFrame, stats: dict, title: str) -> str:
        """Generate clean fallback HTML profile when ydata-profiling is not installed"""
        info = stats.get('dataset_info', {})
        cols = stats.get('column_types', {})
        ...
        <head>
            ...
            <style>
                ...
                .card {{ background: #f1f3f5; padding: 15px; border-radius: 6px; text-align: center; }}
                .card-val {{ font-size: 24px; font-weight: bold; color: #1c7ed6; }}
        ...
        <body>
            <div class="container">
                <h1>{title}</h1>
                <p>Automated Dataset Profile Summary (Fallback Mode)</p>
                
                <div class="grid">
                    <div class="card"><div>Total Rows</div><div class="card-val">{info.get('n_rows', 0):,}</div></div>
                    <div class="card"><div>Total Columns</div><div class="card-val">{info.get('n_columns', 0)}</div></div>
                    <div class="card"><div>Numeric Cols</div><div class="card-val">{cols.get('numeric', 0)}</div></div>
                    <div class="card"><div>Categorical Cols</div><div class="card-val">{cols.get('categorical', 0)}</div></div>
                    <div class="card"><div>Missing Cells %</div><div class="card-val">{info.get('missing_cells_perc', 0):.2f}%</div></div>
                </div>
